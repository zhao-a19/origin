...
static void set_time(struct ntptime *new)
{
    ...
    tv_set.tv_sec += 8 * 60 * 60; //wangjunlei add for UTC
...
static int check_source6(int data_len, struct sockaddr *sa_source, unsigned int sa_len, struct ntp_control *ntpc)
{
    struct sockaddr_in6 *sa_in = (struct sockaddr_in6 *)sa_source;
    (void) sa_len;  /* not used */
    if (debug) {
        printf("packet of length %d received\n", data_len);
        if (sa_source->sa_family == AF_INET6) {
            printf("Source: INET Port %d host ipv6\n", ntohs(sa_in->sin6_port));
        } else {
            printf("Source: Address family ipv6\n");
        }
    }
    /* we could check that the source is the server we expect, but
     * Denys Vlasenko recommends against it: multihomed hosts get it
    ...
    (void) ntpc; /* not used */
#endif
    if (NTP_PORT != ntohs(sa_in->sin6_port)) {
        return 1;  /* fault */
    }
    return 0;
}
...
static void stuff_net_addr6(struct in6_addr *p, char *hostname)
{
    struct hostent *ntpserver;
...
static void setup_receive6(int usd, short port)
{
...
static void setup_transmit(int usd, char *host, short port, struct ntp_control *ntpc)
{
    ...
    memcpy(ntpc->serv_addr, &(sa_dest.sin_addr), 64); /* XXX asumes IPv4 */
    sa_dest.sin_port = htons(port);
    if (connect(usd, (struct sockaddr *)&sa_dest, sizeof sa_dest) == -1)
    {perror("connect"); exit(1);}
}

static void setup_transmit6(int usd, char *host, short port, struct ntp_control *ntpc)
{
    struct sockaddr_in6 sa_dest;
    memset(&sa_dest, 0, sizeof sa_dest);
    sa_dest.sin6_family = AF_INET6;
    stuff_net_addr6(&(sa_dest.sin6_addr), host);
    memcpy(ntpc->serv_addr, &(sa_dest.sin6_addr), 64);
...
static void primary_loop6(int usd, struct ntp_control *ntpc)
{
    fd_set fds;
...
#define sizeof_incoming (sizeof incoming_word)
    ...
    for (;;) {
        ...
        if (pack_len < 0) {
            ...
            get_packet_timestamp(usd, &udp_arrival_ntp);
            if (check_source6(pack_len, &sa_xmit, sa_xmit_len, ntpc) != 0) continue;
            if (rfc1305print(incoming_word, &udp_arrival_ntp, ntpc, &error) != 0) continue;
            /* udp_handle(usd,incoming,pack_len,&sa_xmit,sa_xmit_len); */
        } else {
            printf("Ooops.  pack_len=%d\n", pack_len);
...
int main(int argc, char *argv[])
{
    ...
    short int udp_local_port = NTP_PORT; /* default of 0 means kernel chooses.0---> NTP_PORT mod by wjl 20171117*/
    ...
    ntpc.goodness = 0;
    ntpc.cross_check = 0; /*1 --> 0     mod by wjl 20171117*/
    ...
    if (iptype == 6) {
        ...
        setup_transmit6(usd, hostname, NTP_PORT, &ntpc);
        primary_loop6(usd, &ntpc);
    } else if (iptype == 4) {
