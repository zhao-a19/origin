...
void print_hex(const uint8_t *data, int len) {
	SampledValues_t* sv_pdu = 0;
	asn_dec_rval_t rval;

	rval = ber_decode(NULL, &asn_DEF_SampledValues, (void **)&sv_pdu, data, 2048);
	if (rval.code != RC_OK) {
		fprintf(stderr, "ber_docode failed.\n");
		return;
	}

	xer_fprint(stdout, &asn_DEF_SampledValues, sv_pdu);
	asn_DEF_SampledValues.free_struct(&asn_DEF_SampledValues, sv_pdu, 0);
}
