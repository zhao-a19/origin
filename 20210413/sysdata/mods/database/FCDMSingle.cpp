...
//sdata是包括ip头部的数据包
bool CDM::DoMsg(unsigned char *sdata, int slen, char *cherror, int *pktchange, int bFromSrc)
{
    if (bFromSrc == 1)
    {
...
bool CDM::DoSrcMsg(unsigned char *sdata, int slen, char *cherror)
{
...
bool CDM::DoDstMsg(unsigned char *sdata, int slen, char *cherror)
{
    return true;
}

bool CDM::DecodeRequest(unsigned char *sdata, int slen, char *cherror)
{
    if (slen <= 5)
    {
        return false;
    ...
    memset(m_SqlOperName, 0, sizeof(m_SqlOperName));
...
bool CDM::FindSql(unsigned char *sdata, int slen, char *sql_com, int &sqllen)
{
...
bool CDM::DecodeOper(char *csql, int sqllen, char *coper, char *cpara)
{
    if (sqllen < 6)
    {
        return false;
...
bool CDM::AnalyseCmdRule(char *chcmd, char *chpara, char *cherror)
{
    if (g_debug)
    {
    ...
    for (int i = 0; i < m_service->m_cmdnum; i++)
    {
        if (strcasecmp(chcmd, m_service->m_cmd[i]->m_cmd) == 0)
        {
                                   ...
                                   strlen(chpara)) == E_COMM_OK)
            {
                ...
                {
                    printf("==DM_PERM_FORBID==\n");
                    sprintf(cherror, "%s", DM_PERM_FORBID);
                }
