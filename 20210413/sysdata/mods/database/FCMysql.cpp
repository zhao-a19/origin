...
//sdata是包括ip头部的数据包
bool CMYSQL::DoMsg(unsigned char *sdata, int slen, char *cherror, int *pktchange, int bFromSrc)
{
    if (bFromSrc == 1)
    {
...
bool CMYSQL::DoSrcMsg(unsigned char *sdata, int slen, char *cherror)
{
...
bool CMYSQL::DoDstMsg(unsigned char *sdata, int slen, char *cherror)
{
    return true;
}

bool CMYSQL::DecodeRequest(unsigned char *sdata, int slen, char *cherror)
{
...
bool CMYSQL::FindSql(unsigned char *sdata, int slen, char *sql_com, int &sqllen)
{
...
bool CMYSQL::DecodeOper(char *csql, int sqllen, char *coper, char *cpara)
{
    if (sqllen < 6)
    {
        return false;
...
bool CMYSQL::AnalyseCmdRule(char *chcmd, char *chpara, char *cherror)
{
    if (g_debug)
    {
    ...
    for (int i = 0; i < m_service->m_cmdnum; i++)
    {
        if (strcasecmp(chcmd, m_service->m_cmd[i]->m_cmd) == 0)
        {
                                   ...
                                   strlen(chpara)) == E_COMM_OK)
            {
                ...
                {
                    printf("==MYSQL_PERM_FORBID==\n");
                    sprintf(cherror, "%s", MYSQL_PERM_FORBID);
                }
