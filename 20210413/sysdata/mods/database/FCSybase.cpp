...
//sdata是包括ip头部的数据包
bool CSYBASE::DoMsg(unsigned char *sdata, int slen, char *cherror, int *pktchange, int bFromSrc)
{
    if (bFromSrc == 1)
    {
...
bool CSYBASE::DoSrcMsg(unsigned char *sdata, int slen, char *cherror)
{
...
bool CSYBASE::DoDstMsg(unsigned char *sdata, int slen, char *cherror)
{
    return true;
}

bool CSYBASE::DecodeRequest(unsigned char *sdata, int slen, char *cherror)
{
...
bool CSYBASE::FindSql(unsigned char *sdata, int slen, char *sql_com, int &sqllen)
{
...
bool CSYBASE::DecodeOper(char *csql, int sqllen, char *coper, char *cpara)
{
    if (sqllen < 6)
    {
        return false;
...
bool CSYBASE::AnalyseCmdRule(char *chcmd, char *chpara, char *cherror)
{
    if (g_debug)
    {
    ...
    for (int i = 0; i < m_service->m_cmdnum; i++)
    {
        if (strcasecmp(chcmd, m_service->m_cmd[i]->m_cmd) == 0)
        {
                                   ...
                                   strlen(chpara)) == E_COMM_OK)
            {
                ...
                {
                    printf("==SYBASE_PERM_FORBID==\n");
                    sprintf(cherror, "%s", SYBASE_PERM_FORBID);
                }
