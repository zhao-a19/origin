...
CSipNorm::CSipNorm(int i): CSipBase(i)
{
...
/**
 * [CSipNorm::initChannel 通道初始化]
 */
void CSipNorm::initChannel()
{
    BZERO(m_channel);
    for (int i = 0; i < m_maxchannel; i++) {
        sprintf(m_channel[i].myport, "%d", SIP_NORM_STARTPORT + i * 2);
        m_channel[i].able = true;
    }
}

/**
 * [CSipNorm::getOneChannelID 获取一个可用的通道下标ID号]
...
int CSipNorm::getOneChannelID(const char *mrecvip, const char *mrecvport,
...
/**
 * [CSipNorm::delOneChannel 删除一个流媒体通道]
 * @param chanid [通道下标]
 */
void CSipNorm::delOneChannel(int chanid)
{
    channelOper(chanid, false);
}

/**
 * [CSipNorm::addOneChannel 添加一个流媒体通道]
 * @param nodeid [节点ID]
 * @param chanid [通道ID]
 */
void CSipNorm::addOneChannel(int nodeid, int chanid)
{
    channelOper(chanid, true);
}

/**
 * [CSipNorm::channelOper 打开或删除通道]
 * @param chanid [通道ID]
 * @param ifadd  [是否为打开]
 */
void CSipNorm::channelOper(int chanid, bool ifadd)
{
    ...
    /* 清理动态端口链接追踪 */
    sprintf(chcmd, "%s -D -d %s -p udp --dport %d", CONNTRACK, m_tmpip1, atoi(m_channel[chanid].myport));
    system_safe(chcmd);
    PRINT_DBG_HEAD
    print_dbg("chcmd[%s]", chcmd);

    /* 清理动态端口链接追踪 */
    sprintf(chcmd, "%s -D -d %s -p udp --dport %d", CONNTRACK, m_tmpip1, atoi(m_channel[chanid].myport) + 1);
    system_safe(chcmd);
    PRINT_DBG_HEAD
    print_dbg("chcmd[%s]", chcmd);

    /* 如果上级先发送保活端口数据，会先丢弃，直到反方向有数据后恢复 */

    sem_post(g_iptables_lock);
}

/**
 * [CSipNorm::getChannelProxyPort 获取媒体流通道接收端口号]
 * @param nodeid [节点ID]
 * @param chanid [通道ID]
 * @return    [成功返回端口号，失败返回NULL]
 */
const char *CSipNorm::getChannelProxyPort(int nodeid, int chanid)
{
...
/**
 * [CSipNorm::getChannelProxyIP 获取接收视频流的IP]
 * @param  callid [会话ID]
 * @return        [地址指针]
 */
const char *CSipNorm::getChannelProxyIP(const char *callid)
{
    return m_gapoutip;
}

/**
 * [CSipNorm::getChannelOutIP 获取发送视频流的IP]
 * @param  callid [会话ID]
 * @return        [地址指针]
 */
const char *CSipNorm::getChannelOutIP(const char *callid)
{
    return m_gapinip;
}

/**
 * [CSipNorm::dstVideoPrepare 网闸靠近下级平台的一端，视频流通道准备]
 */
void CSipNorm::dstVideoPrepare()
{
    char chcmd[CMD_BUF_LEN] = {0};
    int low = SIP_NORM_STARTPORT;
    int high = SIP_NORM_STARTPORT + m_maxchannel * 2 - 1;
...
/**
 * [CSipNorm::dstStart 网闸靠近下级平台的一端启动函数]
 * @return [成功返回0]
 */
int CSipNorm::dstStart()
{
    CSipBase::dstStart();
    dstVideoPrepare();
    return 0;
...
/**
 * [CSipNorm::srcStart 网闸靠近上级平台的一端启动函数]
 * @return [成功返回0]
 */
int CSipNorm::srcStart()
{
    return CSipBase::srcStart();
}

const char *CSipNorm::getTypeDesc()
{
    return LOG_TYPE_SIP_NORM;
}

/**
 * [CSipNorm::delChannelByCallID 根据callid清除一个视频流通道]
 * @param callid [会话ID]
 */
void CSipNorm::delChannelByCallID(const char *callid)
{
    for (int i = 0; i < m_maxchannel; ++i) {
        if (strcmp(callid, m_channel[i].callid) == 0) {
            PRINT_DBG_HEAD
