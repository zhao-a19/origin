...
/***************************************************************************/
/*                                     */
/*zdb fix bugs: net exception and compile error 2016-07-25    */
/*zdb fix bugs: net exception while read, blocked process 2018-02-27    */
/*zdb surport ipv6:  2019-02-13    */
/*                                     */
/***************************************************************************/
...
static char *version =
    "ftplib(IPv6) Release 4.0.1 13-Feb-2019, copyright 1996-2019, 2013 Thomas Pfau, 2019 Kevin Zhang";

GLOBALDEF int ftplib_debug = 0;

//zdb debug 2016-07-25
//#define _zdbmodify_ fprintf
#define _zdbmodify_(...) //
...
int net_read(int fd, char *buf, size_t len)
{
    while ( 1 ) {
        ...
        if ( c == -1 ) {
            //if ( errno != EINTR && errno != EAGAIN )    //zdb 2019-04-10 bug1450 timeout
            if ( errno != EINTR )
                return -1;
        } else {
            return c;
...
static int socket_wait(netbuf *ctl)
{
    ...
    _zdbmodify_(stderr, "zdb w1\n");
    //fix bug: control stream maybe blocked if net exception
    if ((ctl->idlecb == NULL))
        return 1;
    if (ctl->dir == FTPLIB_WRITE)
        wfd = &fd;
    else if (ctl->dir == FTPLIB_READ)
        rfd = &fd;
    else if (ctl->dir == FTPLIB_CONTROL) {
        _zdbmodify_(stderr, "zdb w2\n");
        rfd = &fd;
    } else {
        if (ftplib_debug) perror("timeout fd");
        _zdbmodify_(stderr, "zdb w3\n");
        return 1;
    }
    //zdb end
...
static int readline(char *buf, int max, netbuf *ctl)
{
    ...
    //zdb add 2016-07-25
    //fix bug：net exception, but can't distinguish timeout from read(data end)
    _zdbmodify_(stderr, "zdb 1 %d\n", ctl->dir);
    //zdb end
    ...
    do {
        ...
        if (!socket_wait(ctl))
            return -1;  //zdb modify 2016-07-25, before is "return retval;"
        _zdbmodify_(stderr, "zdb 2\n");
...
GLOBALDEF int FtpConnect(const char *host, netbuf **nControl)
{
    ...
    //zdb add 2018-02-27
    //bug420 maybe blocked, timeout=10s
    {
        struct timeval stimeout = {10, 0};

        _zdbmodify_(stderr, "zdb timeout\n");

        if (setsockopt(sControl, SOL_SOCKET, SO_RCVTIMEO, (char *)&stimeout, sizeof(stimeout)) < 0) {
            if (ftplib_debug)
                perror("setsockopt");
            net_close(sControl);
            return 0;
        ...
        setsockopt(sControl, SOL_SOCKET, SO_SNDTIMEO, (char *)&stimeout, sizeof(stimeout));
    }
    //zdb add end
    ...
    *nControl = ctrl;
    ctrl->ipv4_6 = FTPLIB_IPV4;
    return 1;
}

//host format [ip6]:port
GLOBALDEF int FtpConnect6(const char *host, netbuf **nControl)
{
    int sControl;
    ...
    if (isdigit(*pnum))
        sin6.sin6_port = htons(atoi(pnum));
    else {
        struct servent *pse;
#if _REENTRANT
        struct servent se;
...
#if 1
    if (inet_pton(AF_INET6, lhost + 1, &sin6.sin6_addr) <= 0) {
        if ( ftplib_debug )
            perror("inet_pton");
        free(lhost);
        return 0;
    ...
    if (connect(sControl, (struct sockaddr *)&sin6, sizeof(sin6)) == -1) {
        if (ftplib_debug)
            perror("connect");
        net_close(sControl);
        return 0;
...
static int FtpOpenPort(netbuf *nControl, netbuf **nData, int mode, int dir)
{
    ...
    }
    {
        ...
        _zdbmodify_(stderr, "zdb data timeout\n");

        if (setsockopt(sData, SOL_SOCKET, SO_RCVTIMEO, (char *)&stimeout, sizeof(stimeout)) < 0) {
            if (ftplib_debug)
                perror("setsockopt");
            net_close(sData);
            return -1;
        ...
        setsockopt(sData, SOL_SOCKET, SO_SNDTIMEO, (char *)&stimeout, sizeof(stimeout));
    }
    if (nControl->cmode == FTPLIB_PASSIVE) {
...
#include <stddef.h>
static int FtpOpenPort6(netbuf *nControl, netbuf **nData, int mode, int dir)
{
    int sData;
    union {
        struct sockaddr sa;
    ...
    if (nControl->cmode == FTPLIB_PASSIVE) {
        ...
        unsigned int v[2] = {2, 0};
        sin.in6.sin6_family = AF_INET6;
        if (!FtpSendCmd("EPSV 2", '2', nControl))
            return -1;
        cp = strchr(nControl->response, '(');
        if (cp == NULL)
            return -1;
        cp++;
        //sscanf(cp, "|%u|%64s|%u", &v[0], ip_addr, &v[1]);
        if (*cp == '|') {
            char *pe = cp + 1;
            int step = 0;
            ...
            while (*pe != 0) {
                if ((*cp == '|') && (*pe == '|')) {
                    ...
                    if (((ptrdiff_t)(pe - cp) - 1) != 0) {
                        switch (step) {
                        case 0: v[0] = atoi(cp + 1); break;
                        case 1:
                            if (sizeof(ip_addr) - 1 < (ptrdiff_t)(pe - cp) - 1)
                                strncpy(ip_addr, cp + 1, sizeof(ip_addr) - 1);
                            else
                                strncpy(ip_addr, cp + 1, (ptrdiff_t)(pe - cp) - 1);
                            break;
                        case 2: v[1] = atoi(cp + 1); break;
                        }
                    }
                    cp = pe++;
                    step++;
        ...
        if (v[0] == 1) {
            _zdbmodify_(stderr, "zdb pasv no ipv4\n");
            return -1;
        }

        _zdbmodify_(stderr, "zdb %s\n", ip_addr);
        if (inet_pton(AF_INET6, ip_addr, &sin.in6.sin6_addr) <= 0) {
            unsigned int l = sizeof(sin.in6);
            _zdbmodify_(stderr, "zdb getpeername\n");
            if (getpeername(nControl->handle, (struct sockaddr *)&sin.in6, &l) < 0) {
                return -1;
            }
            inet_ntop(AF_INET6, &sin.in6.sin6_addr, ip_addr, sizeof(ip_addr));
        }
        _zdbmodify_(stderr, "zdb pasv |%u|%s|%u|\n", v[0], ip_addr, v[1]);
        sin.in6.sin6_port = htons(v[1]);

    } else {
        unsigned int l = sizeof(sin.in6);
        if (getsockname(nControl->handle, (struct sockaddr *)&sin.in6, &l) < 0) {
            if (ftplib_debug)
                perror("getsockname6");
            return -1;
        }
    ...
    if (nControl->cmode == FTPLIB_PASSIVE) {
        if (connect(sData, (struct sockaddr *)&sin.in6, sizeof(sin.in6)) == -1) {
            if (ftplib_debug)
                perror("connect6");
            net_close(sData);
            return -1;
        ...
        sin.in6.sin6_port = 0;
        if (bind(sData, (struct sockaddr *)&sin.in6, sizeof(sin.in6)) == -1) {
            if (ftplib_debug)
                perror("bind6");
            net_close(sData);
            return -1;
        ...
        if (getsockname(sData, (struct sockaddr *)&sin.in6, &l) < 0) {
            if (ftplib_debug)
                perror("getsockname6");
            net_close(sData);
            return -1;
        ...
        inet_ntop(AF_INET6, &sin.in6.sin6_addr, ip_addr, sizeof(ip_addr));
        sprintf(buf, "EPRT |2|%s|%u|", ip_addr, ntohs(sin.in6.sin6_port));
        _zdbmodify_(stderr, "%s\n", buf);
...
static int FtpAcceptConnection(netbuf *nData, netbuf *nControl)
{
    ...
    struct timeval tv;
    fd_set mask;
    int rv = 0;     //zdb modify 2016-07-25, not initialized
                        ...
                        netbuf **nData)
{
    ...
    if (nControl->ipv4_6 == FTPLIB_IPV6) {
        if (FtpOpenPort6(nControl, nData, mode, dir) == -1)
            return 0;
    } else {
...
GLOBALDEF int FtpRead(void *buf, int max, netbuf *nData)
{
    ...
    if (nData->buf)
        ...
        i = socket_wait(nData);
        if (i != 1)
            return -2; //zdb modify 2016-07-25, before is "return 0"
...
GLOBALDEF int FtpWrite(const void *buf, int len, netbuf *nData)
{
    ...
    if (nData->buf)
        ...
        //zdb modify 2016-07-25, before is "socket_wait(nData)"
        i = socket_wait(nData);
        if (i != 1)
            return -2;
        //zdb end
                   ...
                   netbuf *nControl, int typ, int mode)
{
...
#ifdef __PRIORITY_H__
    //zkp add 2017-07-04
    //Implementing download priority processing
    int k =  priority_set(((typ == FTPLIB_FILE_WRITE) ||  (typ == FTPLIB_FILE_READ)));
    //zkp add end
#endif
    ...
    if (typ == FTPLIB_FILE_WRITE) {
        ...
        //zdb add 2016-07-25
        //fix bug: net exception, issue is -2
        //but this routine return 1
        if (l == -2) {
            if (ftplib_debug) perror("net exception");
            rv = 0;
        }
        //zdb add end
    }
    ...
    if (localfile != NULL)
        fclose(local);
    _zdbmodify_(stderr, "zdb c1\n");
    FtpClose(nData);
    _zdbmodify_(stderr, "zdb c2\n");
    return rv;
}
...
//zdb add 2017-06-19
/*
 * FtpOpts - features setting
 *
 * return 1 if successful, 0 otherwise
 */
GLOBALDEF int FtpOpts(const char *opts, netbuf *nControl)
{
    char cmd[TMP_BUFSIZ];

    if ((strlen(opts) + 7) > sizeof(cmd))
        return 0;
    sprintf(cmd, "OPTS %s", opts);
    if (!FtpSendCmd(cmd, '2', nControl))
        return 0;
    return 1;
}
