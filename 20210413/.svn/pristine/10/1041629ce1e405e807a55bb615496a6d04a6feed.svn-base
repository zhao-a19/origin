...
extern int g_ipseg;
static void dbsync_tash_state(int32 status, pchar stat);
...
*注释:
*******************************************************************************************/
static int32 dbsync_send_post(pchar url, pchar param, pchar st)
{
    if (st == NULL)
    ...
    PRINT_DBG_HEAD;
    print_dbg("dbsync_tool  http json info :%s->%s", param == NULL ? "null" : param, st);
...
static pchar dbsync_update_json_info(dbsync_task list, char **param, int32 run_st = DBSYNC_TASK_DEFAULT)
{
    ...
    cJSON_AddItemToObject(root, "strategyTableInfos", array);
...
*注释:
*******************************************************************************************/
static int32 dbsync_update_task(pchar param, pchar st)
{
    int32 ret = -1;
    cJSON *root = NULL;
...
*注释:
*******************************************************************************************/
static int32 dbsync_timer(pchar param, pchar st)
{
    int32 ret = -1;
    cJSON *root = NULL;
    memset(st, 0, DBSYNC_HTTPBUF_MAX);
...
static void dbsync_tash_state(int32 status, pchar stat)
{
    ...
    logmgr.WriteSysLog(DBSYNC_INFO, (status == DBSYNC_TASK_SUCCESS) ? DBSYNC_SUCCESS : DBSYNC_FAIL, st_gbk);
    logmgr.DisConnect();
#else
    CLOGMANAGE logmgr;
    if (stat == NULL || stat[0] == '\0')
...
*******************************************************************************************/
#if 1
int32 dbsync_update_taskinfo(int32 taskcnt, pdbsync_task tasks, pchar cfg)
{
    ...
    for (int i = 0; i < taskcnt; i++) {
        if (is_endbsyncsuccess(tasks[i])) {
            ...
            dbsync_udate_file(cfg, tmptable_tmp, "0", i + 1);
...
*注释:
*******************************************************************************************/
static int32 dbsync_log_request(pchar param, pchar st)
{
    int32 ret = -1;
    cJSON *root = NULL;
    memset(st, 0, DBSYNC_HTTPBUF_MAX);
...
*注释:
*******************************************************************************************/
static int32 dbsync_status_repuest(pchar st)
{
    int32 ret = -1;
    cJSON *root = NULL;
    memset(st, 0, DBSYNC_HTTPBUF_MAX);
...
*注释:
*******************************************************************************************/
static void dbsync_write_task_info_old(uint32 taskcnt, pchar bakfile, bool TOTAL, CFILEOP &file, dbsync_time timer, pdbsync_task tasks = NULL)
{
    pchar SYSROOT[2] = {"MAIN", "TaskNum"};
    pchar TASKCFG = "Task";
    char tasktmp[64] = {0};
...
int32 dbsync_back_task_info_old(int32 taskcnt, pdbsync_task tasks, dbsync_time timer, pchar bakfile)
{
    ...
    for (int i = 0; i < taskcnt; i++) {
        ...
        dbsync_write_task_info_old(count, bakfile, false, file, timer, tasks + i);
        count++;
    }
    if (count != 0) {
        dbsync_write_task_info_old(count, bakfile, true, file, timer);
    }
    file.WriteFileEnd();
    return 0;
}