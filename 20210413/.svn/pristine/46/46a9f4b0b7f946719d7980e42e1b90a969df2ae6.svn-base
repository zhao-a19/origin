...
//shell计算
//echo -n 字符串 | gzip -1 | tail -c 8 | head -c 4 | hexdump -e '1/4 "%d" "\n"'
...
{
    uint32 CRC32_data = 0xFFFFFFFF;
    for (uint32 i = 0; i != len; ++i) {
        uint32 t = (CRC32_data ^ buf[i]) & 0xFF;
...
uint16 GetCRC16_CCITT(const puint8 buf, uint32 len)
{
    ...
    while (len-- > 0)
        crc = ccitt_table[((crc >> 8) ^ buf[i++]) & 0xff] ^ (crc << 8);
...
/*******************************************************************************************
*
*               网络源码: http://blog.csdn.net/cp1300/article/details/51443350
*
*******************************************************************************************/
#if 0
