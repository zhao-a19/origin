...
CSipLink::CSipLink(int i): CSipBase(i)
{
    BZERO(m_node);
    m_nodenum = 0;
    m_totalw = 0;
...
/**
 * [CSipLink::initChannel 通道初始化]
 */
void CSipLink::initChannel()
{
    if (m_nodenum > 0) {
        for (int i = 0; i < m_nodenum; i++) {
            while ((m_node[i]->pchannel = new MediaChannel[m_maxchannel]) == NULL) {
                PRINT_ERR_HEAD
                print_err("nodeid[%d] m_maxchannel[%d] new channel fail, retry...",
                          i, m_maxchannel);
                sleep(1);
            }
            ...
            for (int j = 0; j < m_maxchannel; j++) {
                sprintf(m_node[i]->pchannel[j].myport, "%d",
                        SIP_LINK_PORT_OFFSET + m_taskno * m_maxchannel * 2 + j * 2);
...
/**
 * [CSipLink::portAble 判断端口是否可用]
 * @param  port [端口]
 * @return      [可用就返回true]
 */
bool CSipLink::portAble(int port)
{
    bool bflag = true;
...
/**
 * [CSipLink::getOneChannelID 获取一个可用的通道下标ID号]
 ...
 * @return           [通道下标  出错返回负值]
 */
int CSipLink::getOneChannelID(const char *mrecvip, const char *mrecvport,
...
/**
 * [CSipLink::delOneChannel 删除一个流媒体通道]
 * @param nodeid [节点ID]
 * @param chnid  [通道ID]
 */
void CSipLink::delOneChannel(int nodeid, int chanid)
{
    int ret = RESULT_OK;
    ...
    ret = fgap.Del(m_name, m_node[nodeid]->comeip, m_node[nodeid]->goip,
    ...
    if (ret != RESULT_OK) {
        ...
        print_err("del channel ret[%d],rname[%s],comeip[%s],goip[%s],myport[%s],rip[%s],rport[%s]",
                  ret, m_name, m_node[nodeid]->comeip, m_node[nodeid]->goip,
                  m_node[nodeid]->pchannel[chanid].myport,
                  m_node[nodeid]->pchannel[chanid].media_recvip,
                  m_node[nodeid]->pchannel[chanid].media_recvport);
    }
...
/**
 * [CSipLink::addOneChannel 添加一个流媒体通道]
 * @param nodeid [节点ID]
 * @param chanid [通道ID]
 */
void CSipLink::addOneChannel(int nodeid, int chanid)
{
    int ret = RESULT_OK;
    CFgapCmd fgap(m_node[nodeid]->natip, m_node[nodeid]->natport);

    ret = fgap.Add(m_name, m_node[nodeid]->comeip, m_node[nodeid]->goip,
                   m_node[nodeid]->pchannel[chanid].myport,
                   m_node[nodeid]->pchannel[chanid].media_recvip,
                   m_node[nodeid]->pchannel[chanid].media_recvport);
    if (ret != RESULT_OK) {
...
/**
 * [CSipLink::getChannelProxyPort 获取媒体流通道接收端口号]
 * @param nodeid [节点ID]
 * @param chanid [通道ID]
 * @return    [成功返回端口号，失败返回NULL]
 */
const char *CSipLink::getChannelProxyPort(int nodeid, int chanid)
{
...
/**
 * [CSipLink::getChannelProxyIP 获取节点接收视频流的IP]
 * @param  callid [会话ID]
 * @return        [地址指针]
 */
const char *CSipLink::getChannelProxyIP(const char *callid)
{
    PRINT_DBG_HEAD
...
/**
 * [CSipLink::getChannelOutIP 获取节点发送视频流的IP]
 * @param  callid [会话ID]
 * @return        [地址指针]
 */
const char *CSipLink::getChannelOutIP(const char *callid)
{
    PRINT_DBG_HEAD
...
/**
 * [CSipTask::srcStart 网闸靠近上级平台的一端启动函数]
 * @return [成功返回0]
 */
int CSipLink::srcStart()
{
    return CSipBase::srcStart();
}

/**
 * [CSipLink::dstStart 网闸靠近下级平台的一端启动函数]
 * @return [成功返回0]
 */
int CSipLink::dstStart()
{
    CSipBase::dstStart();
    nodeCmdNat();
    return 0;
...
const char *CSipLink::getTypeDesc()
{
    return LOG_TYPE_SIP_LINK;
}

/**
 * [CSipLink::delChannelByCallID 通过会话ID清除一条视频流通道]
 * @param callid [会话ID]
 */
void CSipLink::delChannelByCallID(const char *callid)
{
    PRINT_DBG_HEAD
...
/**
 * [CSipLink::设置转发节点调度命令使用的内部nat]
 * 分析处理SIP的逻辑在网闸靠近上级平台的一侧，而调度服务启动在节点靠近下级平台的一侧
 * 所以，需要网闸内部nat跳转一下
 */
void CSipLink::nodeCmdNat()
{
    char chcmd[CMD_BUF_LEN] = {0};
...
/**
 * [CSipLink::nodeTest 测试节点是否在线]
 * @param  nodeid [节点ID]
 * @return        [在线返回true]
 */
bool CSipLink::nodeTest(int nodeid)
{
    char chcmd[CMD_BUF_LEN] = {0};
    //超时 才重新检测在线状态
...
/**
 * [CSipLink::initWeight 权重初始化]
 */
void CSipLink::initWeight()
{
    PRINT_DBG_HEAD
...
/**
 * [CSipLink::getOneNode 按权重 选择一个节点]
 * @return [节点ID]
 */
int CSipLink::getOneNode()
{
    PRINT_DBG_HEAD
...
/**
 * [CSipLink::resetOneChannel 重置一个通道]
 * @param nodeid [节点ID]
 * @param chanid [通道ID]
 */
void CSipLink::resetOneChannel(int nodeid, int chanid)
{
    if ((nodeid >= 0) && (nodeid < m_nodenum)) {
        if ((chanid >= 0) && (chanid < m_maxchannel)) {
